...
class TestTranslateAgent:
    ...
    def translation_context(self):
        ...
        context.config = {"prompts": {"translator": "Translate the following content"}}
    ...
    def test_is_web_url(self, agent):
        """测试URL识别功能"""
        # 测试HTTP URL
        assert agent.is_web_url("http://example.com")
        assert agent.is_web_url("https://example.com")

        # 测试www开头的URL
        assert agent.is_web_url("www.example.com")

        # 测试域名格式
        assert agent.is_web_url("example.com")
    ...
    def test_translate_url_or_text_with_url(
        self, mock_session, agent, translation_context
    ):
        """测试URL翻译路径"""
        # 模拟网络响应
        mock_response = Mock()
        ...
        with patch.object(agent, "translate") as mock_translate:
            ...
            result = agent.translate_URLOrText(
                translation_context, "Spanish", "http://example.com", Mock()
            )
    ...
    def test_translate_url_or_text_with_text(self, agent, translation_context):
        ...
        with patch.object(agent, "translate") as mock_translate:
            ...
            result = agent.translate_URLOrText(
                translation_context, "French", test_text, Mock()
            )
    ...
    def test_translate_chunking(self, agent, translation_context):
        ...
        with patch.object(agent, "talk_to_LLM_Json") as mock_llm_call:
            ...
            with patch.object(
                agent, "get_legal_info", return_value="Legal disclaimer text"
            ):
                result = agent.translate(
                    translation_context, "German", long_content, Mock()
                )
    ...
    def test_should_refresh(self, agent, temp_files):
        """测试文件刷新判断逻辑"""
        # 目标文件不存在时应该刷新
        assert agent.should_refresh(temp_files["target_file"])
        ...
        # 目标文件存在时不应该刷新
        assert not agent.should_refresh(temp_files["target_file"])

        # force_refresh为True时应该刷新
        assert agent.should_refresh(temp_files["target_file"], force_refresh=True)

    @patch("AgentUtils.tomarkdown.getfilecontent")
    def test_translate_element_source_missing(
        self, mock_getfilecontent, agent, translation_context
    ):
        """测试源文件缺失的情况"""
    ...
    @patch("AgentUtils.tomarkdown.getfilecontent")
    def test_translate_element_target_exists(
        self, mock_getfilecontent, agent, translation_context, temp_files
    ):
        """测试目标文件已存在的情况"""
        # 创建目标文件
        with open(temp_files["target_file"], "w") as f:
    ...
    @patch("requests.Session")
    def test_url_translation_network_error(
        self, mock_session, agent, translation_context
    ):
        """测试URL翻译时的网络错误"""
        # 模拟网络错误
        ...
        with pytest.raises(requests.RequestException):
            agent.translate_URLOrText(
                translation_context, "Japanese", "http://example.com", Mock()
            )
