# BLEU May Out Of Date for tranlation tasks?
...
I used flores200 and pick the 1st sentence in Chinese and English to test.
I defined `sys` as result from DeepSeek, and `sys_0`, **note:`sys_0` is copied from `ref`.**
...
Next, let's see the behavior form BLEU:
...
-----
```
The default result is not good, as BLEU relays on tokenize, which makes sense.
...
-----
```
Well, zh equals to tokenize by char, and yes, it bases langauge. But, which makes me thinking wired as `sys_0` is not got full score as 100.
...
-----
```
As our test case from flores200, so let's see BLEU behaivor with flores200. Emm, it's not perfect either.
...
## Why it may out of date?

Which means, if we just compare with word precision... as 1-gram or 2-gram.... which looks good but lost meaning of sentence. And at meanwhile, if we switch tokenizer, it influences result too much.
